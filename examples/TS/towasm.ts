...
// ===================================================================
//
// Missing relative to full TypeScript (this is a narrow static subset, not a general TS compiler):
//  - generics anywhere: functions, classes, closures, function types
//  - inheritance, interfaces, abstract classes, getters/setters; mutable static class members (a
//    `static readonly` literal is special-cased as a compile-time constant, nothing else)
//  - `any`, async/await, generators, exceptions (try/catch/throw), switch, labeled break/continue
//  - closures: no rest/default/optional params, no self-reference, no `this` in a function expression
//    (only an arrow's lexical `this`), call dispatch only through a bare identifier (not `obj.field(x)`),
//    and a captured variable's mutations aren't shared back with the enclosing scope after creation
//  - arrays: only four monomorphized fixed-length kinds (`number[]`/`boolean[]`/`Uint8Array`/`bigint[]`),
//    no arrays of objects/other arrays, no resizing; `string` is immutable and not indexable
//  - destructuring defaults/rest elements; spread call arguments; nullable number/boolean (no boxing)
//  - most Array/String/Math methods beyond a small self-hosted prelude (`map`/`filter`/etc. need
//    closures and still aren't implemented there)
//  - `console.log(x)` supports exactly one number/boolean argument -- not bigint (a software bignum
//    class, not a real wasm scalar), string, or any object (unreadable at the JS import boundary)
//
...
// known, always a plain `call`. A ctor whose fields are all number/boolean is `struct.new_default`
// (zero-init) into a `this` local, with the body then run as ordinary statements -- a non-null ref field
// has no zero value to default-init with, so a class with any object-typed field instead collects every
// field's value as its `this.field = value` statement is reached, then builds `this` via `struct.new`
// (real values, no zero-init step) once every field has one -- see `emitFieldCollectingCtorBody`. Either
// way the ctor returns `this` at the end. Class names are reserved eagerly (a method can reference its
// own class by name before
...
// Every `lib/*.ts` file gets read and parsed the same way, then concatenated into one flat declaration
// list -- `lib.d.ts`'s ambient `__asm`/`i32`/`i64`/`f32`/`f64` declarations first (shared across every
// other lib file, same as any real ambient .d.ts), `towasm-lib.ts` after. File order otherwise doesn't
// matter: nothing downstream (`LIB_DECL_MAP`, hoisting) has any notion of which file a declaration came
// from, and cross-declaration forward references already have to survive arbitrary same-file ordering.
...
//const LIB_EXPORTED = new Set<string>(...LIB_EXPORTS.filter(n => n.type === 'function_decl' || n.type === 'class_decl').map(n => n.name));
...
// `nullable` only ever applies to `ref`/`arr` -- a raw number/boolean has no null representation without boxing, so `'T | null'`/`'T | undefined'` is only supported when `T` is a class/array/string.
// `{closure}`		is a callable value, represented as a 2-field `{code, env}` struct pair (see `ensureClosureType`).
// `{typeIndex}`	is `{ref}` already resolved to a concrete `types[]` slot -- used only internally, for a
// closure literal's own hidden env-param/env-local `FuncCtx.declare` calls.
// `u32` is not a real wasm value type (wasm has no distinct unsigned value representation, only signed
// vs unsigned *instruction variants* over the same `i32` storage) -- it exists purely at this compiler's
// level, to track "this i32 bit pattern means unsigned" as part of the type itself (see `coerceTop`),
// rather than requiring every producer/consumer of a `Uint32Array` element or `>>>`'s result to
// separately remember to pick `convert_i32_u` over the default-signed `convert_i32_s`.
...
// One shared `{arr: kind}` object per `wasmElement`, not a fresh literal at each of the many call sites
// that need one -- `ctx.local`/`ctx.declareLocal` (see `FuncCtx`) compare a repeat local declaration's
// `WasmType` by `!==` (object identity), so e.g. two separate `a[0] = ...`/`a[1] += ...` statements on the
// same plain-array variable within one function threw "redeclared with different type" before this existed
// (same referential-equality gotcha `classWtype` already exists to avoid for class refs).
...
// A ref-kind array element's own wtype -- same "one shared object, not a fresh literal" reasoning as
// `ARR_WTYPE` just above (and the same bug shape: two separate ref-kind index writes/mutating-array-method
// calls in one function, each building their own `{ref:'any'}` literal, would otherwise throw "redeclared
// with different type" the moment both try to name a scratch local `$new`/`$old`/etc).
...
// Structural, not identity -- two independently-obtained `FuncSig`s for the same TS function type must still compare equal.
...
// `defaults[i]`, if present, is param `i`'s own literal default value -- only ever populated for a real
// user/lib function/method/ctor, consulted by `fillDefaultArgs` at each call site.
// `hasRest`: the last `params` entry is a rest param's own *array* wtype (`...items: T[]` -> `T[]`), not a
// plain param -- call sites bundle their trailing arguments into a real array for it (`emitCallArgs`), the
// same compile-time-count reasoning an array literal's own elements already use.
// `reassignsThis`: this method's own declared body assigns to `this` (`assignsToThis`) -- it's compiled with
// an extra wasm-level result (its own final `this` value, see `case 'return'`'s `appendThisOnReturn`), which
// every call site must consume and write back to the receiver's real lvalue (see `case 'call'`).
...
interface MethodOwner {
	...
	// Property names with a `get`/`set` accessor -- the underlying decl/inline-asm lives in `methodDecls`/
	// `inlineMethods` under `accessorKey('get'|'set', name)`; these sets are just "does `name` have one",
	// consulted by `case 'member'`'s read path and `emitAssignTarget`'s write path before falling back to
...
interface ClassInfo extends MethodOwner {
	...
	ctor?:			FuncInfo;
	// Set only for `Array<T>`/`ReadonlyArray<T>` instantiations (see `ensureClass`'s `arrayBackingKind`) --
	// this instance's real `WasmType` is `ARR_WTYPE[arrayKind]`, not the generic `{ref: name}` a struct-
	// backed class would get (checked by `typeOf`'s own generic-class-ref resolution).
	arrayKind?:		wasmElement;
}
...
class FuncCtx {
	...
	out:	wasm.Instr[]	= [];
	ctorThis?: Local;
	// Real wasm branch depths are relative to the *current* instruction position, not a fixed constant --
	// every `block`/`loop`/`if` this compiler emits pushes its own implicit label, so `break`/`continue`
	// can't just hardcode "1"/"0" the moment either sits inside a nested `if` (found via `lib/number.ts`'s
	// own "`break` isn't supported" workaround comment: never actually exercised before `switch` needed it).
	// `depth` counts every such label currently enclosing the point of emission; `breakTargets`/
	// `continueTargets` record the `depth` value *at* each currently-open loop's/switch's own break/continue
	// label, so `depth - target` at any deeper point gives the correct relative `br` index. `switch` pushes
	// only `breakTargets` (a `continue` inside a `switch` still targets the nearest *enclosing loop*, not the
	// switch itself -- real JS semantics).
	depth = 0;
	breakTargets: number[] = [];
	continueTargets: number[] = [];
	// Set only when this `FuncCtx` is itself a closure literal's own body: `envLocal` is the received env
	// param (already `ref.cast` down to `envTypeIndex`), `fields` maps each captured name to its struct field.
	// A captured name has no real wasm local (see `declareCaptured`) -- reads/writes go through
	// `struct.get`/`struct.set` on `envLocal`, checked before `locals`.
	closureEnv?: { envLocal: Local; envTypeIndex: number; fields: Map<string, { index: number; wtype: WasmType }> };
	// Set only while compiling a `reassignsThis` method's body (see `FuncInfo`'s own comment) -- every
	// `return` in such a method must also leave the *current* value of `this` as one more wasm-level result,
	// on top of whatever it normally returns (or, for a `void`-declared method, as the only result).
	...
	// A captured free variable -- registered with the checker's `scope` only, same as `declareValue`, but
	// with no real wasm local (storage is a `closureEnv` struct field, set up by the caller right after).
	declareCaptured(name: string, tsType: Type) {
		this.scope.addValue(name, tsType);
	}

	// Whether `name` resolves somehow in this context: a real local, or (if this function is itself a
	// closure) one of its own captured fields -- checked on the immediately-enclosing `FuncCtx`, since a
	// transitively-captured name may exist only in `closureEnv.fields`, never in a real local.
	resolvesName(name: string): boolean {
		return this.locals.has(name) || !!this.closureEnv?.fields.has(name);
	}

	// A plain-identifier param is declared directly. A destructured one is declared under a hidden synthetic
	// name (`#param$<i>`); the caller gets back `var_decl` statements binding the pattern's real names off
	// that hidden param, and must run them (via `emitStmt`) once, right before the real body.
...
// `null` parses as a literal; `undefined` is a real (global) identifier, not a keyword -- both need this
// same handling, since there's no standalone `WasmType` for "null of no type" (`ref.null` needs a real heap type).
function isNullLiteral(e: Expr): boolean {
	return (e.type === 'literal' && e.value === null) || (e.type === 'identifier' && e.name === 'undefined');
}

// For an error message only -- a plain identifier param/local needs no further description, a pattern
// gets a generic placeholder rather than trying to render its actual shape.
function describeBinding(t: BindingTarget): string {
	return typeof t === 'string' ? t : t.type === 'array_pattern' ? '[...]' : '{...}';
}

// Desugars a destructuring `BindingTarget` into flat plain-identifier `var_decl`s reading off `valueExpr`
// (must already be side-effect-free -- callers materialize the real value into a scratch local first).
// Defaults and rest elements throw explicitly (no conditional/dynamic-length support yet), rather than
// silently dropping/truncating.
...
// Every name `body` binds directly (its own params + its own `var_decl`s) -- does not descend into a
// nested arrow/function's own body (see `collectFreeVars`, which handles nested literals separately).
...
// Recursively collects every free variable/`this` reachable from `body` into the shared `free` set. A
// nested closure's own bound names merge into `bound` before recursing with the same `free` set, so a
// level-2 closure's reference to a level-0 variable transitively becomes part of level-1's capture set too.
...
// The `WasmType` an already-resolved name has in `ctx` -- a real local, or (if `ctx` is itself a closure)
// one of its own captured fields. Used when building a closure literal's own concrete env struct.
function resolvedWtype(ctx: FuncCtx, name: string): WasmType {
	return ctx.closureEnv?.fields.get(name)?.wtype ?? ctx.lookup(name).wtype;
}


// The `'f64'|'i32'|'u32'` kind a `T[]`/`Array<T>`/`ReadonlyArray<T>`'s element type resolves to, or
// `undefined` otherwise -- shared by `wasmTypeOf`'s and `ownerFor`'s otherwise-identical array-element
// checks. `u32` (e.g. `Array<u32>`) shares `i32`'s physical wasm-GC storage (see this file's own
// `toValType`) but keeps its own owner/typeIndex so unsigned comparisons on elements stay correct.
...
// Picks the tightest real wasm-representable type for a known-integer numeric range: signed `i32` if it
// fits, else unsigned `u32` if it fits that (a `Uint32Array` read, `>>>`'s result, etc.), else `f64` --
// matching real JS `number` semantics for anything wider than a 32-bit wrap.
...
// A direct, unresolved ref to a known `builtinTypes` name is checked before `T.resolve` -- `resolve` would
// otherwise fully expand a hoisted class like `String` into its member list, losing the name this needs.
function wasmTypeOf(t: Type, global: Scope): WasmType | undefined {
	...
	// `integer` alone only means "whole number" -- an unbounded range still needs `f64`, so `min`/`max`
	// must actually be checked, not just the flag.
	...
	// `rangeToType` collapses an exactly-known single-value range (e.g. `-1`, folded via `rangeNeg`) down
	// to a plain `Literal`, not a degenerate `range` -- needs the identical bounds check, or it falls
	// through to `widenLiterals` below and silently widens straight to `f64`.
	if (t.type === 'literal' && typeof t.value === 'number')
		return Number.isInteger(t.value) ? intWasmType(t.value, t.value) : 'f64';

	// A union's own members can each hide an unresolved alias (e.g. towasm-lib.ts's `i32`/`i64`/...
	// pseudo-types) that `resolve()` alone won't dereference (it has no `case 'union'`) -- resolve each
	// member first, then let `combineTypes` merge the now-dereferenced duplicates.
...
// A stable, structural key for memoizing closure-type registration by TS function signature (a closure
// literal has no name), synthesizing the "name" from the signature's own shape.
...
// `WatInstr` and `wasm.Instr` only differ on `block`/`loop`/`if` (a WAT-only `label`) -- none of which a
// flat inline-asm sequence can legitimately contain, so this validates that and returns real `Instr[]`.
...
// Resolves an asm body's scratch `(local ...)`s (declared by name, since the asm text has no enclosing
// function to number them against) to real indices in the caller's function -- `ctx.declare(..., true)`
// both allocates (first call) and shares (later calls) the slot. The common case (`locals` empty) returns
// `instrs` unchanged.
function resolveAsmLocals(instrs: wasm.Instr[], locals: WAT.WatLocal[], ctx: FuncCtx, asm: string): wasm.Instr[] {
	if (!locals.length)
		return instrs;
...
function makeAsmBuiltin(key: string, value: Expr | undefined, builtin: string, index: number, elemKind?: wasmElement, elemDefine?: TypedArrayTag): Builtin<Inline> | undefined {
	try {
		...
		// `elemDefine`, when given, is just one more `defines` entry alongside `this` -- a `(switch $elem
		// ...)` in `asm` resolves immediately here (see `WAT.parseAsmBody`'s own `switch` rule), the same
		// general per-instantiation-constant mechanism `$this` already uses, not the deferred/call-site-
		// driven `$T`-switch path below (see `lib/typedarray.ts`'s `get`/`set`).
		...
		const resolveType = (t: Type): WasmType | undefined => {
			...
			if (elemKind !== undefined) {
				if (t.type === 'array')
					return ARR_WTYPE[elemKind];
				// A bare element: scalar kinds (i32/i64/f32/f64/u32) are that real wasm value type directly;
				// `i16`/`ref` (packed/boxed storage -- `Array<T>` itself never actually uses `i16`, only
				// `Uint8Array`-style typed arrays, kept here for completeness) read back as a real object
				// reference, not a raw number.
				return elemKind === 'i16' ? ARR_WTYPE.i16 : elemKind === 'ref' ? { ref: 'any' } : elemKind;
			}
			if (t.type === 'ref') {
				switch (t.name) {
		...
		// Fixed: no `$T` anywhere (`WAT.isTypeGeneric`, a real structural check, true for `(typecase ...)`
		// too) -- params/result come from `P`/`R` themselves.
		if (!WAT.isTypeGeneric(parsed)) {
			...
			const body = { locals, body: assertFlatInstrs(parsed.body, asm) };
		...
		return (args, ctx) => {
			// The operand's own type already has a direct variant, shared by every operand -- no conversion
			// needed (e.g. an `f32` reaching `Math.sqrt` gets `f32.sqrt`, not widened to f64).
			const t = args[0]?.wtype;
			// No asm arm is ever written for `u32` (it's not a real wasm value type -- see `WasmType`'s own
			// comment) -- falls through to the generic widen-to-`NUMERIC_TYPES` path below like any other
			// type without a direct variant.
			if (typeof t === 'string' && t !== 'void' && t !== 'u32' && args.length === paramCount && args.every(a => a.wtype === t)) {
				const parsed = variants[t];
				if (parsed)
					return { params: args.map(() => t), result: t, inline: resolveAsmLocals(parsed.body, parsed.locals, ctx, asm) };
			}
			// Otherwise (including the empty-args probe call) widen to whichever supported type comes first
			// in `NUMERIC_TYPES` -- declaring that as `params` is what makes the caller's own `emitAs` emit
			// the real conversion; this only ever picks the target type, never converts itself.
			const chosen = NUMERIC_TYPES.find(nt => variants[nt]);
			if (!chosen)
				throw new Error(`towasm: '${builtin}.${key}': no numeric type supports this operation`);
			const parsed = variants[chosen]!;
			return { params: Array(paramCount).fill(chosen), result: chosen, inline: resolveAsmLocals(parsed.body, parsed.locals, ctx, asm) };
		};
	} catch (e) {
...
const builtinArrayOwners: Record<wasmElement, MethodOwner> = {
	...
	// it's a real (non-generic) `ensureClass('String')` instantiation now, see its `arrayBackingKind`.
	i16:	placeholderArrayTypeIndex(0),
	// `Array<T>`'s own instantiations no longer build a `MethodOwner` here -- they're real generic
	// instantiations now (`ensureClass('Array', typeArgs)`, see its `arrayBackingKind`), sharing these same
...
// A plain lib-internal class (e.g. `StringParser`, or `Array` itself now) not already one of the
// specially-represented scalar/string builtins above -- resolved the same generic way a user class is
// (`Array`'s own real array-vs-struct representation is `ensureClass`'s own concern, see `arrayBackingKind`).
...
// Shared by the four equality operators -- `==`/`===` and `!=`/`!==` behave identically here (no implicit
// coercion between distinct `WasmType`s to tell loose/strict apart). `bigint` and `string` are both
// checked by owner, not physical shape -- `{arr:'i16'}` happens to mean "string" today, but nothing stops
// a future physically-`{arr:'i16'}` kind from meaning something else (same reasoning as `bigint`, whose
// own physical kind can't be told apart from `Int32Array`/`boolean[]` at all); other ref types dispatch to
// `ref.eq`. `bigint`'s owner is a stable module-level object (`===` identity works); `string`'s is a real
// `ensureClass('String')` instantiation, rebuilt fresh per compile, so it's checked by name instead.
...
// What a `builtins` entry gets per operand -- both the physical `WasmType` (kind-polymorphic dispatch,
// e.g. i32 vs f64) and the real `MethodOwner` (identity dispatch, e.g. `bigint`/`string` methods).
...
// Looks up one of `LIB_DECLS`' top-level `const X = __asm<P, R>('...')` entries by name (e.g. `%`'s own
// `__towasm_mod`) and resolves it eagerly, so an operator entry can delegate to it directly.
...
export function TStoWasm(ast: TS.Program): wasm.WasmModule {
	...
	// representation as a wasm-GC *array* rather than a struct is `ensureClass`'s own concern, see its
	// `arrayBackingKind`), resolved (and monomorphized/cached, via `ensureClass`) to its `ClassInfo` --
	...
	function typeOf(t: Type): WasmType | undefined {
		...
		if (generic)
			return generic.arrayKind ? ARR_WTYPE[generic.arrayKind] : { ref: generic.name };
		...
		// goes through `resolveGenericClassRef` above at all, only ever through this path). Same `arrayKind`
		// check as the generic branch above, for the same reason: an array-backed class's real `WasmType`
		// is `{arr:kind}`, not the generic `{ref:name}` -- found via `new String(s)`'s result type still
		// wrongly claiming `{ref:'String'}` once some earlier statement in the same compile had already
		// resolved `String` once (before that, `wasmTypeOf`'s own fast path happened to give the right
		// answer by a different route, masking this).
		if (t.type === 'ref') {
			const cls = classes.get(t.name);
			if (cls)
				return cls.arrayKind ? ARR_WTYPE[cls.arrayKind] : { ref: t.name };
	...
	// `this`'s `WasmType`, derived from `owner.thisTsType` on demand rather than stored on `MethodOwner` itself
	function ownerThisType(owner: MethodOwner): WasmType {
		const wt = typeOf(owner.thisTsType);
		if (!wt)
	...
	// itself -- an array-backed class (`Array<T>`, see `ClassInfo.arrayKind`) has a real `ClassInfo` behind
	...
	function emitExpr(e: Expr, ctx: FuncCtx, want?: WasmType): WasmType {
		switch (e.type) {
			...
			case 'new': {
				...
				// `new Array<T>(n)` -- a fresh, zero-filled wasm-GC array. `Array<T>`'s own real (checker-
				// visible) `constructor(n)` just calls the `alloc<T>` intrinsic (`lib/array.ts`), but that
				// intrinsic is only reachable via `case 'call'`'s `ctx.owner`-relative dispatch ([towasm.ts]
				// `namespaceOwner`: `builtinTypes[obj.name]?.owner ?? ... ?? (ctx.owner?.decl.name === obj.name ? ctx.owner : ...)`)
				// -- i.e. only from *inside* one of `Array<T>`'s own methods, not from arbitrary calling code
				// (confirmed by running it: `lib/bigint.ts` calling `Array.alloc<u32>(n)` directly threw
				// `unknown method 'alloc'`). `new Array<T>(n)` needs no such context, so it gets its own
				// small dedicated branch, same as `ArrayBuffer`/`String` above -- resolving the element kind
				// directly from the explicit type argument rather than through `ctx.owner`.
				if (e.callee.type === 'identifier' && e.callee.name === 'Array') {
					const elemType = e.typeArgs?.[0];
					if (!elemType)
						throw new Error("towasm: 'new Array<T>(n)' needs an explicit type argument");
					const kind = arrayElemKind(elemType, global);
					if (!kind)
						throw new Error("towasm: 'new Array<T>(n)' only supports number/boolean/u32/i64/f32-kind elements");
					if (e.arguments.length !== 1 || e.arguments[0].type === 'spread')
						throw new Error("towasm: 'new Array<T>(n)' needs exactly one argument (a length)");
					emitAs(e.arguments[0], ctx, 'i32');
					ctx.emit(I.array.new_default(builtinArrayOwners[kind].typeIndex));
					return ARR_WTYPE[kind];
				}
	...
	function ensureClass(name: string, typeArgs?: Type[]): ClassInfo | undefined {
		...
		// `Array<T>`/`ReadonlyArray<T>`/`String` are wasm-GC *arrays*, not structs -- see the long comment at
		// this function's `typeIndex`-assignment site below for why this is the one class-name-specific
		// fork left here. `String` has no type argument to derive its kind from (always `i16`, real JS
		// strings never being anything else). Computed up front too: `this`'s own `WasmType` inside such an
		// instantiation's methods must resolve to the real `{arr:kind}` array shape (via `thisTsType`/
		// `ownerThisType`/`typeOf`'s ordinary `T[]`/builtin-name handling), not the generic `{ref: key}` a
		// struct-backed class would get.
		const arrayBackingKind = name === 'String' ? 'i16'
			: (name === 'Array' || name === 'ReadonlyArray') && typeArgs?.length ? arrayElemKind(typeArgs[0], global)
			: undefined;
		...
		if (!info) {
			...
			info = { name: key, typeIndex: -1, thisTsType: arrayBackingKind ? (typeArgs?.length ? TS.ArrayType(typeArgs[0]) : T.STRING) : TS.RefType(key), decl, fields: [], fieldIndex: new Map(), methodDecls: new Map(), arrayKind: arrayBackingKind };
...
//						throw new Error(`towasm: field initializers ('${name}.${m.key}') are not supported -- set them in the constructor`);
			...
			// `Array<T>`/`ReadonlyArray<T>`/`String` are wasm-GC *arrays*, not structs -- their physical
			// typeIndex already exists (the per-`wasmElement` registration loop below, `builtinArrayOwners`),
			// reused here (`arrayBackingKind`, computed above) rather than building a fresh (and, since
			// `length` is a real getter now, not a stored field -- empty and wrong) struct. This is the one
			// class-*name*-specific fork left in this function: deriving "is this instantiation array-
			// backed" generically from its own constructor's return expression would need relaxing
			// `ensureCtor`'s "a constructor cannot return a value" restriction too (`Array`'s own
			// `constructor(n) { return Array.alloc(n) as Array<T>; }` can't otherwise even be compiled --
			// confirmed: `case 'new'`'s dedicated `Array`/`String` branches both bypass `ensureCtor` entirely
			// for exactly this reason) -- out of scope here. Ordinary field/type-parameter substitution above
			// needs no Array/String-specific handling at all: `T` inside e.g. `slice(): T[]` substitutes to
			// the real resolved type argument (not a hand-picked stand-in the old `builtinOwner` scheme
			// needed), and `wasmTypeOf`'s existing generic `T[]` handling already resolves that to the right
			// `{arr:kind}` on its own.
			info.typeIndex = arrayBackingKind ? builtinArrayOwners[arrayBackingKind].typeIndex : registerType({ final: true, supertypes: [], type: {
				kind: 'struct',
			...
			const { inlineMethods, asmMethodKeys } = scanInlineMethods(decl, name, info.typeIndex, arrayBackingKind, TYPED_ARRAY_ALIASES[name]?.elem);
	...
	function ensureCtor(cls: ClassInfo): FuncInfo {
		...
		worklist.push(() => {
			const ctx		= new FuncCtx(new Scope(libGlobal), thisWtype);
			...
			if (cls.fields.some(f => typeof f.wtype !== 'string')) {
	...
	const HOIST_EXCLUDE		= new Set(['Array', 'Boolean']);
