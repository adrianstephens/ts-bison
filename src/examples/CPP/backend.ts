...
// overloads, or standard library -- an unsupported construct throws `WasmError` rather than miscompiling.
...
function declaredType(spec: C.DeclSpec<CPP.TypeSpecifierExt>): Val {
	if (spec.type.type !== 'ref')
		throw new WT.WasmError(`unsupported type specifier '${spec.type.type}'`);
	const w = SCALARS[spec.type.name];
	if (!w)
		throw new WT.WasmError(`unsupported type '${spec.type.name}'`);
	return w;
}

// The usual arithmetic conversions, cut down to the scalars above: the widest of the two, float winning.
function commonType(a: Val, b: Val): WT.Scalar {
...
function constOf(t: WT.Scalar, v: number): wasm.Instr {
	const k = WT.notUnsigned(t);
...
export function CPPtoWasm(module: { body: CPP.Definition[] }): wasm.WasmModule {
	...
	function toValType(w: WT.Type): wasm.ValType {
		if (typeof w !== 'string' || w === 'void')
			throw new WT.WasmError('only scalar values are supported');
		return WT.notUnsigned(w);
	}
	...
	function functionParts(d: CPP.Declarator) {
		if (d.type !== 'function' || d.name.type !== 'identifier')
			throw new WT.WasmError('only a plain `name(params)` declarator is supported');
		return {
			name:	d.name.name,
			params: d.params.map(p => {
				// A pack shares `type: 'parameter'` with an ordinary one, so `pack` is what tells them apart.
				if ('pack' in p)
					throw new WT.WasmError('a parameter pack is not supported');
				if (!p.declarator || p.declarator.type !== 'identifier')
					throw new WT.WasmError('only plain named parameters are supported');
	...
	function convert(got: Val, want: Val, ctx: WT.FunctionContext): void {
		if (got === 'void' || want === 'void')
			return;
		const g = WT.notUnsigned(got), w = WT.notUnsigned(want);
		if (g === w)
			return;
	...
	function binaryInstr(op: string, t: WT.Scalar): wasm.Instr {
		const k = WT.notUnsigned(t);
		...
		throw new WT.WasmError(`unsupported operator '${op}'`);
	}
	...
	function typeOf(e: Expr, ctx: WT.FunctionContext): Val {
	...
	function emitExpr(e: Expr, ctx: WT.FunctionContext, want?: Val): Val {
		switch (e.type) {
			case 'literal': {
				if (typeof e.value !== 'number')
					throw new WT.WasmError('only numeric literals are supported');
			...
			case 'identifier': {
				...
				if (!local)
					throw new WT.WasmError(`unknown identifier '${e.name}'`);
			...
			case 'unary': {
				...
				if (e.operator !== '-')
					throw new WT.WasmError(`unsupported unary '${e.operator}'`);
			...
			case 'assign': {
				if (e.target.type !== 'identifier' || e.operator)
					throw new WT.WasmError('only plain `name = value` assignment is supported');
				const local = ctx.lookup(e.target.name);
				if (!local)
					throw new WT.WasmError(`unknown identifier '${e.target.name}'`);
			...
			case 'call': {
				if (e.callee.type !== 'identifier')
					throw new WT.WasmError('only a direct call by name is supported');
				const fn = funcs.get(e.callee.name);
				if (!fn)
					throw new WT.WasmError(`unknown function '${e.callee.name}'`);
				if (fn.params.length !== e.arguments.length)
					throw new WT.WasmError(`'${e.callee.name}' takes ${fn.params.length} arguments, not ${e.arguments.length}`);
		...
		throw new WT.WasmError(`unsupported expression '${e.type}'`);
	}

	function truthy(e: Expr, ctx: WT.FunctionContext): void {
		const t = emitExpr(e, ctx);
		if (t === 'void')
			throw new WT.WasmError("a 'void' value has no truth value");
		if (isFloat(t))
			ctx.emit(constOf(t, 0), binaryInstr('!=', t));
		else if (WT.notUnsigned(t) === 'i64')
			ctx.emit(I.i64.eqz, I.i32.eqz);
	}

	// ---- statements -----------------------------------------------------

	function emitStmt(s: Stmt, ctx: WT.FunctionContext, result: Val): void {
		switch (s.type) {
			case 'empty':
				return;
			...
			case 'declaration': {
				...
				if (wtype === 'void')
					throw new WT.WasmError("a local cannot have type 'void'");
				for (const d of s.initDeclarators ?? []) {
					const decl = 'declarator' in d ? d.declarator : d;
					if (decl.type !== 'identifier')
						throw new WT.WasmError('only a plain named declarator is supported');
					const init = 'initializer' in d ? d.initializer : undefined;
					if (init && 'type' in init && init.type === 'initializer_list')
						throw new WT.WasmError('an initializer list is not supported');
			...
			case 'break':
				if (!ctx.breakTargets.length)
					throw new WT.WasmError("'break' outside of a loop");
				ctx.emitBreak();
				return;

			case 'continue':
				if (!ctx.continueTargets.length)
					throw new WT.WasmError("'continue' outside of a loop");
				ctx.emitContinue();
				return;
		...
		throw new WT.WasmError(`unsupported statement '${s.type}'`);
	}
	...
	out.code = defs.map((def, i) => {
		...
		const ctx = new WT.FunctionContext(name);
		params.forEach(p => ctx.declareLocal(p.name, p.wtype));
		WT.withCatch(() => emitStmt({ type: 'block', body: def.body.body }, ctx, result), name)();
