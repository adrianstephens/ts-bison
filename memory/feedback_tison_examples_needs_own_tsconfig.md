...
description: "verifying tison/src/examples/TS/*.ts (backend.ts, checker.ts, etc) requires running tsc from src/examples/, not the tison package root -- the root tsconfig doesn't check that directory at all"
metadata: 
  node_type: memory
  type: project
...
root therefore silently checks *nothing* under `src/examples/TS/` (backend.ts, checker.ts,
...
backend.ts edits, including a real `Property 'ref' does not exist on type '{...} | {...}'` error
...
backend.ts's own parser rather than tsc -- not a gap, working as intended). `test/tsconfig.json`
...
`checker.ts`/`backend.ts`/`module-loader.ts`/`tsw.ts` edits -- including a real, committed
excess-property error in `backend.ts` (`ensureObjectShape`'s local `fields` array missing `optional`)
