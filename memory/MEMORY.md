...
- [two-tier gates](feedback_two_tier_gates.md) — fast gates while iterating, full set once before the commit; measured 10.1 gate runs per commit. **`test-towasm.ts` and `test-checker.ts` read `dist/` — `npm run examples` first, on both sides of an A/B**
